...
| **Note history** | Notes synced to `wip_tasks` via DB trigger (`trg_sync_latest_note`). Direct `wip_notes` insert nodes (`Log Initial Note`, `Log Status Change Note`, `Log Backfill Note`) removed — note logging delegated to DB trigger or a separate flow |
...
- **Daily WIP Sync: Filter Daily WIP Meeting simplified** — regex-based `daily_meeting_title_pattern` matching removed from this node (pattern filtering is now handled upstream by the Google Calendar node); node now passes all received calendar items through
- **Daily WIP Sync: cascade-close flow removed** — `IF: cascade tickets?` and `Cascade Close Tickets` nodes dropped; `Aggregate Summary Counts` no longer attempts to count or reference cascade results; `tickets_auto_closed` now counts completed WIP actions with a `wip_task_id`
- **Daily WIP Sync: workflow timezone corrected** — `Asia/Vientiane` → `Asia/Ho_Chi_Minh`; all `last_updated` timestamps switched from `new Date().toISOString()` (UTC) to `DateTime.now().setZone('Asia/Ho_Chi_Minh').toISO()`
- **Helpdesk Intake: `new_business` work_type added** — same 4-value system propagated through both ticket-generator LLM prompts, output schemas (both `WIP Fields Output Schema` and `Thread WIP Fields Output Schema` changed from nullable 2-value to required 4-value enum), and all JS `work_type` compute expressions; `new_business` from LLM now takes priority over client-based `client_work` inference
- **Helpdesk Intake: wip_notes logging nodes removed** — `Log Initial Note`, `Log Status Change Note`, `Log Backfill Note` Supabase insert nodes dropped; each path now routes directly to `Mark Message as Read` (note logging delegated to DB trigger or separate flow)
- **Helpdesk Intake: all timestamps use VN timezone** — `latest_note_at`, `last_updated`, and ticket-ID date prefix now use `toLocaleString('sv-SE', {timeZone:'Asia/Ho_Chi_Minh'})` pattern for `+07:00`-offset ISO strings instead of UTC `toISOString()`
